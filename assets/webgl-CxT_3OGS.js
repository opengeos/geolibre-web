...
set_result(result);
  ...
  `,B=new T(s,{vs:M,shaderAssembler:K,defines:P,modules:f,bufferLayout:l,vertexCount:1,instanceCount:i.length,attributes:d,feedbackBufferMode:"interleaved",outputs:a.varyings});s.statsManager.getStats(Z).get(J).incrementCount(),B.run({inputBuffers:d,outputBuffers:{[a.varyings[0]]:i.offset===0?o:{buffer:o,byteOffset:i.offset,byteLength:i.byteLength}}}),v&&y.recycle(v)}function Q(t){return Array.isArray(t)?t.map((e,r)=>[`x${r}`,e]):Object.entries(t)}function I(t,e,r,n=!1,i=e){let u="",o="";for(let s=0;s<r;s+=4){const a=Math.min(r-s,4),f=m(e,a,n);u+=`in ${f} a${t}_${s};
...
`,Ee=({inputs:t,output:e,target:r})=>(_({module:{name:"row_length",vs:xe},inputs:t,output:e,operationType:"float32",outputBuffer:r}),{success:!0}),ye=async({inputs:t,output:e,target:r})=>{const{segments:n}=t,i=r.device,u=b("result",e.type,e.size),o=n.type,s=Y(n,o,i),a=new T(i,{vs:`#version 300 es
