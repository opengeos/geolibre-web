import{n as h}from"./scriptingApi-B2htKi0a.js";import{n as g,t as b}from"./pyodide-config-BYNk__GD.js";var j=`"""GeoLibre in-app Python console API.
...
`,d=new Set;function w(e){return d.add(e),()=>d.delete(e)}function p(e){for(const n of d)n(e)}var f=null,v=6e4;async function y(e,n,t){if(n())return;let o;const a=new AbortController,r=setTimeout(()=>a.abort(),v);try{const i=await fetch(e,{signal:a.signal});if(clearTimeout(r),!i.ok)throw new Error(`HTTP ${i.status}`);o=await i.arrayBuffer()}catch(i){throw new Error(`Failed to load the Pyodide runtime script from ${e}.`,{cause:i})}finally{clearTimeout(r)}const s=URL.createObjectURL(new Blob([o],{type:"text/javascript"}));try{await new Promise((i,_)=>{const l=document.createElement("script");l.src=s,l.onload=()=>{if(l.remove(),n()){i();return}_(new Error(`Pyodide script loaded but ${t} is not defined; the content at ${e} may be incorrect.`))},l.onerror=()=>{l.remove(),_(new Error(`Failed to load the injected Pyodide runtime script from ${e}.`))},document.head.appendChild(l)})}finally{URL.revokeObjectURL(s)}}function x(e){return f??=(async()=>{await y(`${e}pyodide.js`,()=>typeof window.loadPyodide=="function","window.loadPyodide"),!g(e)&&await y(`${e}pyodide.asm.js`,()=>typeof window._createPyodideModule=="function","window._createPyodideModule")})().catch(n=>{throw f=null,n}),f}var c=null;async function P(e){const n=b();if(p("Downloading Python runtime"),await x(n),!window.loadPyodide)throw new Error("Pyodide failed to initialize.");const t=await window.loadPyodide({indexURL:n});p("Setting up GeoLibre");const o={...h(e),loadPackage:a=>t.loadPackage(a)};return t.registerJsModule("_geolibre_js",o),t.runPython(j),t}function u(e){return c??=P(e).catch(n=>{throw c=null,n}),c}var m=Promise.resolve();function S(e,n){const t=m.then(()=>L(e,n));return m=t.catch(()=>{}),t}async function L(e,n){const t=await u(e);let o="";const a=r=>{o+=r.endsWith(`
`)?r:`${r}
`};t.setStdout({batched:a}),t.setStderr({batched:a});try{const r=await t.runPythonAsync(n);if(r!=null){const s=r;try{a(typeof s.toString=="function"?s.toString():String(r))}finally{typeof s.destroy=="function"&&s.destroy()}}return{output:o,error:null}}catch(r){return{output:o,error:r instanceof Error?r.message:String(r)}}finally{t.setStdout(),t.setStderr()}}async function N(e,n,t){const o=(await u(e)).globals.get("_geolibre_complete");try{const a=o(n,t);return JSON.parse(a)}finally{o.destroy?.()}}function k(e){return{getController:e}}export{S as a,w as i,k as n,u as r,N as t};
