import{Bs as y,Vs as D,fs as V,hs as T,mc as G,ms as N,ps as X,vc as j,yc as p}from"./maplibre-Mo3KD5jN.js";function m(t,e,r=!1){if(r)return e===1?"float":`vec${e}`;switch(t){case"uint8":case"uint16":case"uint32":return e===1?"uint":`uvec${e}`;case"sint8":case"sint16":case"sint32":return e===1?"int":`ivec${e}`;default:return e===1?"float":`vec${e}`}}function z(t,e,r=!1){let n;if(r)switch(t){case"uint8":n="unorm8";break;case"sint8":n="snorm8";break;case"uint16":n="unorm16";break;case"sint16":n="snorm16";break;case"float32":n="float32";break;default:throw new Error(`Unsupported normalized vertex format for ${t}`)}else n=t;return e===1?n:e===3&&!n.startsWith("float32")&&!n.endsWith("32")?`${n}x3-webgl`:`${n}x${e}`}function $(t){switch(t[0]){case"u":return"0u";case"s":return"0";default:return"0."}}function q(t,e){switch(t){case"uint8":case"uint16":case"uint32":return`${Math.trunc(e)}u`;case"sint8":case"sint16":case"sint32":return`${Math.trunc(e)}`;default:return Number.isInteger(e)?`${e}.0`:`${e}`}}function H(t){switch(t){case"uint8":return"r8uint";case"sint8":return"r8sint";case"uint16":return"r16uint";case"sint16":return"r16sint";case"uint32":return"r32uint";case"sint32":return"r32sint";case"float32":return"r32float";default:throw new Error(`Unsupported WebGL gather texture format for ${t}`)}}function Le(t){return t}function W(t){switch(t){case"uint32":return"usampler2D";case"sint32":return"isampler2D";case"float32":return"sampler2D";default:throw new Error(`Unsupported WebGL gather sampler type for ${t}`)}}var Z="GPGPU Operation Counts",J="Transform Runs",K=new j;function v({module:t,elementWise:e=!1,expression:r,inputs:n,output:i,operationType:u=i.type,outputBuffer:o}){const s=o.device,a=b("result",i.type,i.size,i.normalized),f=[t,a],l=[],d={},g=m(i.type,1,i.normalized),x=m(u,1,i.normalized);let L="",_=null;const P={TYPE:x,RESULT_LEN:i.size.toString()},w=Q(n);for(const[c,h]of w)f.push(I(c,h.type,h.size,h.normalized,u)),l.push(A(c,h)),h instanceof N?d[c]=h.buffer:(_=_||T.createOrReuse(s,o.byteLength),d[c]=_),L+=`TYPE ${c}[${h.size}]; get_${c}(${c});
...
`,le=({inputs:t,output:e,target:r})=>(v({module:{name:"fround",vs:ue},inputs:t,output:e,operationType:"uint32",outputBuffer:r}),{success:!0});function C(t,e,r){const n=W(r),i=m(e,1),u=Array.from({length:t.size},(o,s)=>`  v[${s}] = ${i}(texelFetch(source_values_texture, ivec2(${s}, rowIndex), 0).r);`).join(`
...
`,ve=({inputs:t,output:e,target:r})=>(v({module:{name:"row_dot",vs:he},inputs:t,output:e,operationType:"float32",outputBuffer:r}),{success:!0}),_e=`void equalAll(in TYPE x[X_LEN], in TYPE y[Y_LEN], out uint result[1]) {
...
`,pe=({inputs:t,output:e,target:r})=>(v({module:{name:"equalAll",vs:_e},inputs:t,output:e,operationType:e.type==="uint32"?t.x.type:e.type,outputBuffer:r}),{success:!0}),xe=`void row_length(in TYPE x[X_LEN], out float result[1]) {
  float sum = 0.0;
  for (int i = 0; i < X_LEN; i++) {
...
`,Ee=({inputs:t,output:e,target:r})=>(v({module:{name:"row_length",vs:xe},inputs:t,output:e,operationType:"float32",outputBuffer:r}),{success:!0}),ye=async({inputs:t,output:e,target:r})=>{const{segments:n}=t,i=r.device,u=b("result",e.type,e.size),o=n.type,s=Y(n,o,i),a=new y(i,{vs:`#version 300 es
